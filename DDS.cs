...
namespace DotNetDDS
{
    ...
    {
        public static (DDS_HEADER, byte[]) FromStream(Stream stream)
        {

            var header = ReadFrom(stream);
            var data = new byte[stream.Length - stream.Position];
            stream.Read(data, 0, data.Length);
            return (header, data);
        }
        public static DDS_HEADER ReadFrom(Stream stream)
        {

            var br = new BinaryReader(stream);
            var magic = br.ReadUInt32();
            var header = DDS_HEADER.FromStream(stream);
            return header;
        }
        public static void WriteTo(this DDS_HEADER self, Stream s)
        {
            self.dwSize = DDS_HEADER.SIZE;
            self.ddspf.dwSize = DDS_PIXELFORMAT.SIZE;
            s.Write(DDS_HEADER.MAGIC, 0, 4);
            var b = self.ToBytes();
